...
const FamilyTreeCanvas: React.FC<FamilyTreeCanvasProps> = ({
      ...
      if (selectedNId) {
        ...
        if (focusRelations.spouse) {
          currentEdges
            ...
            .forEach((e) =>
              relevantNodeIds.add(
                e.source === selectedNId ? e.target : e.source,
              ),
            );
        }
  ...
  const handleAutoLayout = useCallback(async () => {
    ...
    const { nodes: layoutedNodes, edges: layoutedEdges } =
      layoutStrategy.layout([...nodes], [...edges]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setNodes(() => {
        const { nodes: highlightedNodes } = applyHighlight(
          flowNodes,
          flowEdges,
          selectedNodeId,
          selectedEdgeId,
      ...
      setEdges(() => {
        const { edges: highlightedEdges } = applyHighlight(
          flowNodes,
          flowEdges,
          selectedNodeId,
          selectedEdgeId,
  ...
  const processConnection = useCallback(
    async (targetNodeId: string) => {
      if (!connectionStartRef.current || !connectionStartRef.current.nodeId)
        return;
  ...
  const updateSelectionAndStyles = useCallback(
    (nId: string | null, eId: string | null) => {
      setSelectedNodeId(nId);
      setSelectedEdgeId(eId);

      setNodes((currentNodes) => {
        const { nodes: styledNodes } = applyHighlight(
          currentNodes,
          edges,
          nId,
          eId,
      ...
      setEdges((currentEdges) => {
        ...
        const { edges: styledEdges } = applyHighlight(
          nodes,
          edgesWithSelection,
          nId,
          eId,
