...
import { deleteMember, deleteParentChildRelationship, deleteSpouseRelationship, getFamilies } from '../services/api';
...
const Home: React.FC = () => {
  ...
  const [confirmState, setConfirmState] = useState<{ isOpen: boolean; memberId: string | null; edgeId?: string | null; edgeType?: string | null }>({
    isOpen: false,
    memberId: null
  });
  ...
  const handleConfirmDelete = async () => {
      if (confirmState.memberId) {
          ...
          try {
             ...
             setConfirmState({ isOpen: false, memberId: null, edgeId: null, edgeType: null });
             handleRefresh();
  ...
  if (loading) {
    return <div className="h-full w-full flex items-center justify-center"><Loader2 className="animate-spin" /></div>;
  }
  ...
  return (
    <div className="h-full w-full flex flex-col overflow-hidden isolate">
      ...
      <div className="flex-1 flex overflow-hidden relative z-0">
             ...
             {family ? (
                 <>
                     ...
                     <FamilyTreeCanvas 
                        ...
                        readOnly={isReadOnly}
                     />
                     <div className="absolute top-4 left-4 bg-white/80 p-2 rounded text-xs text-gray-500 pointer-events-none" dangerouslySetInnerHTML={{ __html: t('relation.instructions') }}>
                     </div>
                 </>
             ) : (
        ...
        {/* Sidebar */}
        <div className={`transition-all duration-300 ease-in-out ${selectedMember || selectedEdge ? 'w-80' : 'w-0'} overflow-hidden border-l`}>
             {selectedMember && (
                 <MemberDetail 
      ...
      <ConfirmDialog
        ...
        message={confirmState.edgeId ? t('relation.confirm_delete') : t('member.confirm_delete')}
