import SharedFamilyRow from '@/components/SharedFamilyRow';
import { useAuth } from '@/hooks/useAuth';
import { deleteUser, getFamilyName, getUsers, removeCollaborator, updateCollaboratorRole, updateUserRole } from '@/services/api';
import { User } from '@/types';
import { ChevronLeft, ChevronRight, Search, Shield, ShieldAlert, Trash2, User as UserIcon } from 'lucide-react';
...
const Admin: React.FC = () => {
    ...
    const fetchUsers = useCallback(async () => {
        try {
            ...
            await Promise.all(Array.from(sharedFamilyIds).map(async (fid) => {
    ...
    const handleRoleUpdate = async (familyId: string, userId: string, newRole: 'viewer' | 'editor' | 'admin') => {
        try {
    ...
    const handleRemoveCollaborator = async (familyId: string, userId: string) => {
        try {
            ...
            toast.success(t('admin.collaborator_removed', { defaultValue: 'Collaborator removed' }));
            // Update local state
            setUsers(users.map(u => {
                if (u.id === userId) {
                    return {
                        ...u,
                        shared_families: u.shared_families?.filter(sf => sf.family_id !== familyId)
                    };
                }
                return u;
            }));
            ...
            toast.error(t('admin.remove_collaborator_failed', { defaultValue: 'Failed to remove collaborator' }));
        }
    };
            ...
            <div className="mb-6 flex justify-between items-center">
                <div className="relative max-w-sm w-full">
                    ...
                    <input
                        ...
                        placeholder={t('admin.search_placeholder', { defaultValue: 'Search by name or email...' })}
                        value={searchTerm}
                        ...
                        {currentUsers.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="px-6 py-10 text-center text-gray-500">
                                    {t('admin.no_users_found', { defaultValue: 'No users found matching your search.' })}
                                </td>
                            </tr>
                        ) : (
                                                ...
                                                <div className="flex flex-wrap gap-1 mt-1">
                                                    {user.shared_families.map(sf => (
                                                        <SharedFamilyRow 
                                                            ...
                                                            onUpdateRole={(familyId, role) => handleRoleUpdate(familyId, user.id, role)}
                                                            onRemove={(familyId) => handleRemoveCollaborator(familyId, user.id)}
                                                        />
                                                    ))}
                                                </div>
                                            </div>
                        ...
                        <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p className="text-sm text-gray-700">
                                    {t('admin.showing', { defaultValue: 'Showing' })} <span className="font-medium">{indexOfFirstItem + 1}</span> {t('admin.to', { defaultValue: 'to' })} <span className="font-medium">{Math.min(indexOfLastItem, filteredUsers.length)}</span> {t('admin.of', { defaultValue: 'of' })} <span className="font-medium">{filteredUsers.length}</span> {t('admin.results', { defaultValue: 'results' })}
                                </p>
                            </div>
                            <div>
