...
import { Family, GraphData, Member, ParentChildRelationship, SpouseRelationship, User } from '../types';
...
export const getFamilyStats = async (familyId: string) => {
  const response = await api.get<{ member_count: number }>(`/members/count?family_id=${familyId}`);
...
export const createSpouseRelationship = async (member1Id: string, member2Id: string) => {
...
export const createParentChildRelationship = async (parentId: string, childId: string, type: 'father' | 'mother') => {
  const response = await api.post<ParentChildRelationship>('/relationships/parent-child', {
    parent_id: parentId,
    child_id: childId,
...
export const updateSpouseRelationship = async (id: string, data: { marriage_date?: string }) => {
  const response = await api.put<SpouseRelationship>(`/relationships/spouse/${id}`, data);
  return response.data;
};
...
export const inviteCollaborator = async (familyId: string, email: string, role: 'viewer' | 'editor' | 'admin') => {
    const response = await api.post(`/families/${familyId}/invite`, { email, role });
    return response.data;
};

export const updateCollaboratorRole = async (familyId: string, userId: string, role: 'viewer' | 'editor' | 'admin') => {
...
export interface ImportMember extends Omit<Member, 'id' | 'created_at' | 'updated_at'> {
    original_id?: string;
...
export const loginUser = async (email: string, password: string = 'password') => {
...
export const registerUser = async (email: string, name: string, password: string = 'password') => {
    try {
...
export const createUser = async (email: string, name: string) => {
    ...
    try {
        const response = await api.post<User>('/users/', { email, name, password: 'password' });
        return response.data;
    } catch (error) {
...
export const updateUserRole = async (id: string, isSuperuser: boolean) => {
    const response = await api.put<User>(`/users/${id}/role`, { is_superuser: isSuperuser });
    return response.data;
};

// Access Requests
export const createAccessRequest = async (familyId: string, userId: string) => {
    const response = await api.post('/families/' + familyId + '/access-requests', { family_id: familyId, user_id: userId });
    return response.data;
};

export const getPendingAccessRequests = async (userId: string) => {
    const response = await api.get('/families/access-requests/pending?user_id=' + userId);
    return response.data;
};

export const approveAccessRequest = async (requestId: string) => {
    const response = await api.put(`/families/access-requests/${requestId}/approve`);
    return response.data;
};

export const rejectAccessRequest = async (requestId: string) => {
    const response = await api.put(`/families/access-requests/${requestId}/reject`);
    return response.data;
};
