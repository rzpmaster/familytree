...
const PropertyPanel: React.FC<PropertyPanelProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      await updateSpouseRelationship(edge.id, {
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
          <div className="text-sm text-blue-800 font-medium mb-1">
            {t("relation.spouse_relationship", {
              defaultValue: "Spouse Relationship",
            })}
          </div>
          <div className="text-xs text-blue-600">ID: {edge.id}</div>
        </div>
        ...
        <div>
          ...
          {!readOnly && (
            <p className="mt-1 text-xs text-gray-400">
              {t("common.leave_empty_if_unknown", {
                defaultValue: "Leave empty if unknown",
              })}
            </p>
          )}
        </div>
