...
const MemberNode = memo(({ data, selected }: MemberNodeProps) => {
  ...
  const handleDelete = (e: React.MouseEvent) => {
    ...
    const event = new CustomEvent('deleteMember', { detail: { id: data.id } });
  ...
  return (
    ...
    >
      ...
      <div className={clsx("p-2 border-b flex justify-between items-center rounded-t-md shrink-0", isMale ? "bg-blue-50" : "bg-pink-50")}>
        <div className="flex items-center gap-2 overflow-hidden">
            <User size={16} className={clsx("shrink-0", isMale ? "text-blue-500" : "text-pink-500")} />
            <span className="font-bold text-sm text-gray-700 truncate" title={data.name}>{data.name}</span>
        </div>
      </div>
      
      <div className="p-3 text-xs text-gray-500 space-y-1 flex-1 overflow-hidden">
        <div>{t('member.gender')}: {t(`member.${data.gender}`)}</div>
        {data.birth_date && <div>{t('member.birth_date')}: {data.birth_date}</div>}
        {data.death_date && <div>{t('member.death_date')}: {data.death_date}</div>}
        {data.birth_place && <div className="truncate">{t('member.birth_place')}: {data.birth_place}</div>}
      </div>
