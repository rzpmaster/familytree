...
def read_families(user_id: str = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
@router.get("/{family_id}/collaborators", response_model=List[schemas.FamilyCollaborator])
...
@router.put("/{family_id}/collaborators/{user_id}", response_model=schemas.FamilyCollaborator)
def update_collaborator_role(family_id: str, user_id: str, update: schemas.FamilyCollaboratorUpdate, db: Session = Depends(get_db)):
...
def create_access_request(family_id: str, request: schemas.AccessRequestCreate, db: Session = Depends(get_db)):
...
def update_family(family_id: str, family: schemas.FamilyUpdate, db: Session = Depends(get_db)):
