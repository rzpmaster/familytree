...
def create_region(region: schemas.RegionCreate, db: Session = Depends(get_db)):
    ...
    if region.member_ids:
        ...
        members = db.query(models.Member).filter(models.Member.id.in_(region.member_ids)).all()
...
def update_region(region_id: str, region: schemas.RegionUpdate, db: Session = Depends(get_db)):
    db_region = db.query(models.Region).filter(models.Region.id == region_id).first()
    if db_region is None:
        raise HTTPException(status_code=404, detail="Region not found")
    ...
    if region.member_ids is not None:
        ...
        if to_remove:
            db.query(models.Member).filter(models.Member.id.in_(to_remove)).update({models.Member.region_id: None}, synchronize_session=False)
